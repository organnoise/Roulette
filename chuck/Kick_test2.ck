...
fun void setButtonStateInit(){
    ...
    <<<"determine">>>;
    for(0 => int i; i < 16; i++){
        serial.send(0,i,kick.settings[i][1]$int);
    }
}
...
fun void setButtonState(){
    while(true){
        ...
        serial.send(0,beat,kick.settings[clock.beat][1]$int);
...
fun void getButtonState(){
    while(true){
        ...
        kick.settings[moduleNum][1]$int=> int flip;
        !flip => kick.settings[moduleNum][1];
